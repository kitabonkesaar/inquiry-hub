...
import quickLeadImg from '@/assets/bus-2.jpg';
...
export function LeadFormSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Persist lead locally for the admin panel
    try {
      const payload = {
      ...
      const existing = localStorage.getItem('rentAnyBus_leads');
      const parsed = existing ? JSON.parse(existing) : [];
      localStorage.setItem('rentAnyBus_leads', JSON.stringify([...parsed, payload]));
    } catch {
      // ignore storage errors, still allow user to submit
    }
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-foreground">Any travel dates</p>
                  <p className="text-muted-foreground text-xs">One-way or round trip</p>
                </div>
              </div>
            </div>

            <div className="relative mt-4 w-full max-w-md rounded-2xl overflow-hidden shadow-xl border border-border">
              <img
                src={quickLeadImg}
                alt="Comfortable RentAnyBus coach interior"
